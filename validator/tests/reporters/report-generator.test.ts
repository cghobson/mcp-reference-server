...
import { ReportGenerator } from '../../src/reporters/report-generator.js';
import {
  createServerInfo,
...
describe('ReportGenerator', () => {
  ...
  describe('generate', () => {
    ...
    it('should calculate summary correctly with all tools passing', () => {
      ...
      expect(report.summary.implementedTools).toBe(ONX_TOOLS.length);
      expect(report.summary.missingTools).toHaveLength(0);
    });
    ...
    it('should identify missing tools', () => {
      ...
      expect(report.summary.missingTools.length).toBeGreaterThan(0);
      expect(report.summary.implementedTools).toBeLessThan(ONX_TOOLS.length);
    });
    ...
    it('should merge functional results into tool results', () => {
      ...
      expect(getOrdersResult?.functionalValid).toBe(false); // Has one failure
    ...
    it('should count passed and failed checks correctly', () => {
      ...
      // 2 passing checks + 1 existence check = 3 passed
      // 1 failing check = 1 failed
    ...
    describe('compliance level', () => {
      ...
      it('should be "partial" when any amount of tools are implemented', () => {
        ...
        const toolResults: ToolValidationResult[] = ONX_TOOLS.map((tool, index) =>
          createToolValidationResult(tool, {
            exists: index < 2, // Only 2 tools exist
            errors: index >= 2
              ? [createFailingResult(tool, 'tool-exists', `Missing: ${tool}`)]
              : [],
          })
    ...
    describe('score calculation', () => {
      ...
      it('should calculate percentage correctly', () => {
        ...
        // 1 existence + 1 passing = 2 passed
        // 1 failing = 1 failed
        // Score = 2/3 = 67%
