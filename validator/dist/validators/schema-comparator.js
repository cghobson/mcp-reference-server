...
export function compareSchemas(toolName, endpointSchema, canonicalSchema, path = '') {
    const results = [];
    ...
    if (canonicalSchema.items && canonicalSchema.type === 'array') {
        ...
        if (!endpointSchema.items) {
            ...
            const itemResults = compareSchemas(toolName, endpointSchema.items, canonicalSchema.items, itemPath);
...
function compareConstraints(toolName, endpointSchema, canonicalSchema, path) {
    const results = [];
    // Numeric constraints
